...
def gettime(timestamp):
    ...
    now = datetime.strptime(
        str(datetime.now()), '%Y-%m-%d %H:%M:%S.%f').strftime('%d-%m-%Y:%S-%M-%H')
    ts = timestamp
    tf = '%d-%m-%Y:%S-%M-%H'
...
def writetodb():
    ...
    try:
        if method == "insert":
            ...
            sql = "INSERT INTO "+tbl + \
                "("+attrib_names+") VALUES ("+attrib_values+")"
...
@app.route('/api/v1/usersss',methods=['GET'])
def view():
    return Response(status=200)
...
@app.route('/api/v1/users', methods=['PUT','POST','DELETE','HEAD','OPTIONS'])
def add_user():
    #res2=requests.get('http://0.0.0.0:6000/api/v1/_count')
    no_of_req[0]+=1
    ...
    try:
        ...
        if(validationP(password) == True):
            ...
            task1 = {"method": "select", "table": "user",
                     "columns": ["*"], "where": "username='"+userm+"'"}

            res1 = requests.post(
                'http://0.0.0.0:6000/api/v1/db/read', json=task1)

            if res1.status_code == 404:

                task2 = {"method": "insert", "insert": (userm, password), "column": (
                    "username", "password"), "table": "user"}
                res = requests.post(
                    'http://0.0.0.0:6000/api/v1/db/write', json=task2)
...
@app.route('/api/v1/users/<string:username>', methods=['DELETE','PUT','POST','GET','HEAD','OPTIONS'])
def del_user(username):
    ...
    try:
        ...
        task1 = {"method": "select", "table": "user", "columns": [
        ...
        if res1.status_code == 404:
            ...
            task2 = {"method": "delete", "insert": (
                username), "column": ("username"), "table": "user"}

            res3 = requests.post(
                'http://0.0.0.0:6000/api/v1/db/write', json=task2)
            task3 = {"method": "delete", "insert": (
                username), "column": ("created_by"), "table": "cride"}
            res4 = requests.post(
                'assn3-983027369.us-east-1.elb.amazonaws.com:6000/api/v1/db/write', json=task3)

            task4 = {"method": "delete", "insert": (
                username), "column": ("ujname"), "table": "ride"}
            res5 = requests.post(
                'assn3-983027369.us-east-1.elb.amazonaws.com:6000/api/v1/db/write', json=task4)
...
@app.route('/api/v1/users', methods=['GET','POST','DELETE','HEAD','OPTIONS'])
def list_user():
    #res2=requests.get('http://52.21.35.178:80/api/v1/_count')
    print("USERS")
    no_of_req[0]+=1
    ...
    #global cnt
    #cnt=cnt+1
    task1 = {"method": "selectuser", "table": "user",
...
def clear_db():
    ...
    #cnt=cnt+1
    res = requests.post(
        'http://54.152.143.19/api/v1/db/clear')
    if res.status_code == 200:
        task2 = {"method": "clearuser", "insert": (
            "username"), "column": ("username"), "table": "user"}
        ...
        # res=requests.post('http://34.238.29.50:60006000/api/v1/db/write',json=task2)
...
def count_req():
	...
	'''print("COUNTTT",request.base_url)

	if(request.base_url=="http://52.21.35.178:80/api/v1/_count"):
		#no_of_req[0]+=1
		return Response(json.dumps(no_of_req),mimetype='application/json',status=200)
	else:
        	return Response(json.dumps(no_of_req),mimetype='application/json',status=200)'''
